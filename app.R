...
cleansing <- function(polling_data) {
  ...
  edited_data <- polling_data %>%
    ...
    # Assuming year is a character column. Convert non-numeric entries
    # to NA temporarily.
    mutate(
      year = ifelse(grepl("[P|p]refer", year) | !grepl("^\\d+$", year),
      NA_character_, 
      year)) %>%
