...
all_datasets <- list(
  ...
  read.csv("raw_polls/BOP November 2025 Poll Coded.csv"),
  read.csv("raw_polls/BOP March 2026 Poll.csv")
)
...
cleansing <- function(polling_data) {
  ...
  edited_data <- polling_data %>%
    ...
    # Assuming year is a character column. Convert non-numeric entries to
    # NA temporarily.
    mutate(year = ifelse(
...
raking <- function(dataset) {
  ...
  unname(as.vector(raking_weights[[1]])) # return weights
}

# Makes a topline for a given dataset and feature, saves it, and then adds
# it to a pdf
make_topline <- function(dataset, feature) {
  ...
  # For a simple random sample, using "ids=~1" indicates no stratification
  # or clustering.
  ...
  # Calculate toplines using svytable and return
...
make_crosstab <- function(dataset, feature1, feature2) {
  ...
  # Create a Survey Design with Weights
  # For a simple random sample, using "ids=~1" indicates no 
  # stratification or clustering.
  # The "weights" argument specifies the weight column from the data.
  design <- svydesign(ids = ~1, data = transformed_dataset, weights = ~weight)
...
ui <- fluidPage(
  ...
  fluidRow(
    column(3,
      selectInput(
        "pollnum", "Select Poll Number", choices = seq_along(all_datasets)
      ),
      radioButtons(
        "analysis_choice", "Choose analysis type:", 
        c("topline" = "topline", "crosstab" = "crosstab")
      ),
      conditionalPanel(
  ...
  fluidRow(column(12, tableOutput("table_display")))
)
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    # Exclude "weight" and "date"
    choices <- setdiff(colnames(data), c("weight", "date"))

    updateSelectInput(session, "column_topline", choices = choices)
    updateSelectInput(
      session, "column1", choices = choices, selected = "gender"
    )
    updateSelectInput(session, "column2", choices = choices, selected = "year")
  })
  ...
  output$table_display <- renderTable({
    ...
    if (input$analysis_choice == "topline") {
      ...
      selected_col <- input$column_topline
      data <- make_topline(data, selected_col) %>%
        as.data.frame() %>%
      ...
      col1 <- input$column1
      col2 <- input$column2
      data <- make_crosstab(data, col1, col2) %>%
        as.data.frame() %>%
  ...
  output$plot <- renderPlot({
    ...
    if (input$analysis_choice == "topline") {
      ...
      ggplot(
        data = topline,
        aes(
          x = factor(!!sym(selected_col), levels = input$level_order_topline),
          y = Percent
        )
      ) +
      ...
      crosstab <- make_crosstab(dataset, col1, col2) %>%
        ...
        # Arrange rows based on the sortable input
        mutate(across(where(is.numeric), ~.x * 100))
